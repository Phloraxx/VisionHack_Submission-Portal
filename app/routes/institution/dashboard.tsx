...
export const action = secureAction({ roles: ["institution"] }, async ({ formData, user, pb, intent }) => {
  const institution = await getInstitutionForUser(pb, user.id);
  ...
  switch (intent) {
    case "invite-lead": {
      ...
      const existingTeam = await pb
        ...
        .getFirstListItem(
          pb.filter(
            "leaderUserId = {:leaderUserId} && institutionId = {:institutionId}",
            { leaderUserId: leadUserId, institutionId: institution.id },
          ),
          { fields: "id" },
        )
        .catch(() => null);
      if (existingTeam) {
        return fail({ error: "A team already exists for this lead at your institution", status: 409 });
      }
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            {atCapacity && (
              ...
              >
                This institution has reached its maximum of {institution.maxTeams} teams. Contact the event admin to increase capacity.
              </div>
            )}
            <Form method="post">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="rounded-md border border-info/30 bg-info/8 px-3 py-2.5 text-xs text-info leading-relaxed">
              Invite as many teams as you want, but you can only shortlist up
              to {institution.maxTeams} teams for final submission.
            </div>
          </CardContent>
        </Card>
      </div>
