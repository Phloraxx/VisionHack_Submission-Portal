			...
			<div className="grid gap-6 lg:grid-cols-3">
				...
				<div className="lg:col-span-2 space-y-6">
					...
					<Card>
						...
						<CardContent className="space-y-4">
							<div className="space-y-3">
								<div>
									<p className="text-sm font-medium text-muted-foreground mb-1">Idea Title</p>
									{team.idea_title ? (
										<p className="text-sm whitespace-pre-wrap">{team.idea_title}</p>
									) : (
										<p className="text-sm text-muted-foreground italic">Not provided.</p>
									)}
								</div>
								<div>
									<p className="text-sm font-medium text-muted-foreground mb-1">Idea Description</p>
									{team.idea_desc ? (
										<p className="text-sm whitespace-pre-wrap">{team.idea_desc}</p>
									) : (
										<p className="text-sm text-muted-foreground italic">Not provided.</p>
									)}
								</div>
								<div>
									...
									{team.idea_tech_stack ? (
										<p className="text-sm whitespace-pre-wrap">{team.idea_tech_stack}</p>
									) : (
										<p className="text-sm text-muted-foreground italic">Not provided.</p>
									)}
								</div>
							...
							<div className="text-xs text-muted-foreground flex items-center gap-1">
								...
								Created: {team.created ? new Date(team.created).toLocaleString() : "—"}
							</div>
						</CardContent>
					</Card>
					...
					<Card>
						...
						<CardContent>
							{members.length === 0 ? (
								<p className="text-sm text-muted-foreground italic">No members registered.</p>
							) : (
								<div className="grid gap-3 sm:grid-cols-2">
									{members.map((member, idx) => (
										<div key={member.id} className="border rounded-lg p-3 space-y-1.5">
							...
							{questionnaire ? (
								<div className="text-sm space-y-3">
									{Object.entries(questionnaire)
										.filter(
											([key]) => !(QUESTIONNAIRE_EXCLUDE_KEYS as readonly string[]).includes(key),
										)
										.map(([key, value]) => (
											<div key={key} className="flex justify-between gap-3 border-b pb-1">
						...
						<CardContent className="space-y-3 text-sm">
							{inst && (
								<div>
									...
									{inst.code && (
										<p className="text-xs text-muted-foreground font-mono">{inst.code}</p>
									)}
								</div>
							)}
							<Separator />
