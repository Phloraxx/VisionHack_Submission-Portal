...
vi.mock("~/lib/auth.server", () => ({
	...
	setAuthCookie: vi.fn(
		() => "auth=token; HttpOnly; Secure; SameSite=Strict; Path=/; Max-Age=604800",
	),
}));
...
function extractStatusBody(result: unknown) {
	if (result && typeof result === "object" && "init" in result && "data" in result) {
		const { init, data: payload } = result;
		const status =
			init && typeof init === "object" && "status" in init ? Number(init.status) : undefined;
		const body =
...
function makeRequest(url = "http://localhost:5173/admin/test", body?: URLSearchParams) {
...
function makeArgs(request: Request, params: Record<string, string> = {}) {
...
describe("secureAction", () => {
	...
	it("returns 403 JSON when role is insufficient", async () => {
		mockRequireRole.mockRejectedValue(new Response(null, { status: 403, statusText: "Forbidden" }));
		const handler = vi.fn();
		const action = secureAction({ roles: ["admin"] }, handler);
	...
	it("defaults intent to empty string when missing", async () => {
		...
		const req = makeRequest("http://localhost:5173/admin/test", new URLSearchParams());
		await action(makeArgs(req));
